...
def run_training_step_with_backprop(src_batch, tgt_batch, parameter_list, model_params, optimizer_state, step_number, config):
    ...
    loss.backward()
    optimizer_state = apply_adam_step_to_all_parameters(parameter_list, optimizer_state, learning_rate, beta1, beta2, epsilon)
    return loss.item()
...
def run_training_loop_for_steps(batches, parameter_list, model_params, optimizer_state, num_steps, config):
    ...
    for step in range(num_steps):
        lossi.append(
            run_training_step_with_backprop(
                batches[step % len(batches)][0], batches[step % len(batches)][1], 
                parameter_list, model_params, 
                optimizer_state, step + 1, config
            )
        )
    return lossi
...
def append_tokens_to_beam_sequences(beam_sequences, beam_indices, token_ids):
    ...
    return torch.concat((beam_sequences[beam_indices], token_ids.unsqueeze(1)), 1)
...
def mark_finished_beams(token_ids, finished_flags, end_token_id):
    ...
    return (token_ids == end_token_id) + finished_flags
...
def select_best_finished_beam(finished_sequences, finished_scores, alpha):
    ...
    scores = [finished_scores[i] / compute_length_penalty(len(finished_sequences[i]), alpha) for i in range(len(finished_scores))]
    return {
        'sequence': finished_sequences[torch.argmax(torch.tensor(scores))],
        'score': max(scores)
    }
